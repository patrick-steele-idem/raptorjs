        ...
        PageOptimizer.prototype = {
            ...
            optimizePageCached: function(context, cacheKey, options) {
                ...
                function cacheOptimizedPage(optimizedPage) {
                    ...
                    for (var k in context.debug) {
                        if (context.debug.hasOwnProperty(k)) {
                            optimizedPage.debug[k] = context.debug[k];
                        }
                    }

                    optimizedPage.debug.cacheKey = cacheKey;
                    optimizedPage.debug.cacheContainerKey = cache.key;

                    var renderContext = context.renderContext;
                    if (renderContext) {
                        var request = renderContext.getAttributes().request;
                        if (request) {
                            optimizedPage.debug.isSecure = request.secure;
                            optimizedPage.debug.url = request.url;
                            optimizedPage.debug.host = request.host;
                            optimizedPage.debug.port = request.port;
                        }
                    }
                ...
                if (!optimizedPage) {
                    ...
                    options.context.debug = context.debug;
